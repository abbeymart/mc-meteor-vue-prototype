<template>
    <div>
        <div class="w3-container w3-blue w3-round-medium w3-margin-8">
            <h4><i class="glyphicon glyphicon-thumbs-up"></i> {{mcLabel.testOrderCancelMessage}}</h4>
        </div>
        <form class="w3-margin-8">
            ...
            <div class="row">
                ...
                <div class="col-sm-6">
                    <button class="btn btn-primary" @click.prevent="testCancelMessage">{{mcLabel.testOrderCancelMessage}}
                    </button>
                </div>
            </div>
        </form>
                    ...
                    Meteor.call( 'getOrder', queryParams, currentItemId, userToken, ( error, result ) => {
                        if( error ) {
                            ...
                            this.orderItems  = result.value.filter(item => {
                                const isItemCancel = item.orderItems.some(cancel => {
                                    return cancel.isCancel;
                                });
                                return (item.isCancel || isItemCancel);
                            });
                        } else {
                            this.isMessage   = true;
            ...
            testCancelMessage() {
                ...
                if( userToken ) {
                    if( validOrder ) {
                        ...
                        // this.validateErrors = '';
                        Meteor.call( 'sendCancelEmail', this.orderNumber, cancelType, userToken, ( error, result ) => {
                            if( error ) {
                                this.isMessage   = true;
