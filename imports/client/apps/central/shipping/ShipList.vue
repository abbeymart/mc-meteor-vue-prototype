<template>
    <div>
        <div class="w3-container w3-blue w3-round-medium w3-margin-8">
            <h4><i class="glyphicon glyphicon-th-list"/> {{mcLabel.shipList}}</h4>
        </div>
        <div class="w3-container w3-margin-8">
            ...
            <table class="w3-table w3-striped w3-border w3-hoverable" id="mcShipList">
                ...
                <tr v-for="item in shipItems" :key="item._id" @click.prevent="viewItem(item)">
                    ...
                    <td>{{item.shipStatus}} [{{item.shipType}}]</td>
                    <td>
...
<script>
    ...
    export default {
        ...
        methods   : {
            ...
            viewItem( item ) {
                // Set current item
                this.$store.dispatch('central/setShipItem', item);
                // Session.set( 'currentShipItem', item );
