<template>
    <div>
        <div class="w3-container w3-blue w3-round-medium w3-margin-8">
            <h4><i class="glyphicon glyphicon-thumbs-up"></i> {{mcLabel.docSettingDetail}}</h4>
        </div>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            const item          = this.$store.getters[ 'central/getRoleItem' ];
            if( item ) {
                this.itemId = item._id;
                this.role   = {
                    roleGroup  : item.roleGroup,
                    roleCat    : item.roleCat,
                    roleService: item.roleService,
                    roleDesc   : item.roleDesc,
                    roleCreate : item.roleCreate,
                    roleRead   : item.roleRead,
                    roleUpdate : item.roleUpdate,
