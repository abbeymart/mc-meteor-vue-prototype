<template>
    <div>
        <div class="w3-container w3-blue w3-round-medium w3-margin-8">
            <h4><i class="glyphicon glyphicon-thumbs-up"></i> {{mcLabel.docDetail}}</h4>
        </div>
            ...
            <div class="row">
                <div class="col-sm-6">
                    <div class="form-group">
                        <label for="docFolder">{{mcLabel.folder}}</label>
                        <select class="form-control" v-model="doc.docFolder" id="docFolder" required>
                            <option disabled value="">{{mcLabel.select}} {{mcLabel.folder}}</option>
                            <option v-for="item in folderItems" :value="item._id">{{ item.folderName }}</option>
                        </select>
                        <span v-if="validateErrors" class="help-block w3-yellow">{{validateErrors.docFolder}}</span>
                    </div>
                    <div class="form-group">
                        <label for="docCat">{{mcLabel.cat}}</label>
                        <select class="form-control" v-model="doc.docCat" id="docCat" required>
                            <option disabled value="">{{mcLabel.select}} {{mcLabel.cat}}</option>
                            <option v-for="item in docCategories" :value="item._id">{{ item.codeName }}</option>
                        </select>
                        <span v-if="validateErrors" class="help-block w3-yellow">{{validateErrors.docCat}}</span>
                    </div>
                    <div class="form-group">
                ...
                <div class="col-sm-6">
                    ...
                    <div class="form-group">
                        <label for="docActivity">{{mcLabel.activity}}</label>
                        <input type="text" class="form-control" id="docActivity"
                               :placeholder="mcLabel.activity" maxlength="255"
                               v-model="doc.docActivity">
                        <span v-if="validateErrors"
                              class="help-block w3-yellow">{{validateErrors.docActivity}}</span>
                    </div>
                    <div class="form-group">
...
<script>
    ...
    import { validateDocument } from '/imports/lib/utils/ValidateRecord';
    ...
    export default {
        ...
        methods   : {
            ...
            saveItem() {
                ...
                if( !this.doc.docOwner && !this.$lo.isEmpty( userInfo) ) {
                    this.doc.docOwner = userInfo._id;
                }

                // validate inputs: TODO: review/fine-tune
                const errors = validateDocument( this.doc );
                        ...
                        for( let i = 0, ln = files.length; i < ln; i++ ) {
                            Images.insert( files[ i ], ( err, fileObj ) => {
                                ...
                                if( fileObj ) {
                                    ...
                                    // TODO: docUrl (Document management)
                                    ...
                                    Meteor.call( 'saveDoc', this.doc, this.itemId, userToken, ( error, result ) => {
                                        if( error ) {
                                            this.isMessage   = true;
        ...
        created() {
            ...
            const item          = this.$store.getters[ 'central/getDocItem' ];
            if( item ) {
                this.itemId = item._id;
                this.doc    = {
                    docCat     : item.docCat,
                    docType    : item.docType,
                    docFolder  : item.docFolder,
                    docName    : item.docName,
                    docFile    : item.docFile,
                    docOwner   : item.docOwner,
                    docActivity: item.docActivity,
                    docRef     : item.docRef,
                    docUrl     : item.docUrl,
