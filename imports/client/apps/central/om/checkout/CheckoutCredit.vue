<!--TODO: trade-details > pay/invoice > order/receipt > track -->
<template>
    <div>
            ...
            <div>
                <div class="w3-container">
                    ...
                    <div class="col-sm-3">
                        ...
                        <div>
                            <!--TODO: review formatCost() usage, instead of CostLabels-->
    ...
    export default {
        ...
        methods   : {
            ...
            updateTradeItems( orderId ) {
                ...
                const defaultFolder = this.tradeFolder;
...
//                }
                ...
                stripeTest.createToken( this.card )
                    .then( ( result ) => {
                        if( result.error ) {
                            // Inform the user if there was an error
                            this.isMessage   = true;
                            this.pageMessage = 'unable proceed with payment: ' + JSON.stringify( result.error );
                            return;
                        }
                            ...
                            if( this.$lo.isEmpty( errors ) ) {
                                ...
                                Meteor.call( 'saveChargeCustomerTest', payInfo, payToken, userToken, ( error,
                                    ...
                                    if( error ) {
                                        ...
                                        const payMsg                                 = (payResult || !this.$lo.isEmpty( payResult )) ? `${payResult.code}: ${payResult.value.status} :: ${payResult.value.outcome.seller_message} :: ${payResult.value.outcome.network_status}` : 'No payment message - contact Support';
                    ...
                    .catch( ( err ) => {
                        this.isMessage   = true;
                        this.pageMessage = 'unable to get payment-token: ' + JSON.stringify( err );
                    } );
            },
            livePayment() {
                const userToken = this.$auth.getToken();
                ...
                // get client payment token, then process payment or report payment-token-error
                stripeLive.createToken( this.card )
                    .then( ( result ) => {
                        if( result.error ) {
                            // Inform the user if there was an error
                            this.isMessage   = true;
