<template>
    <!--TODO: list cancelled orders (full) and cancelled order-items (partial)-->
    <div>
        <div class="w3-container w3-blue w3-round-medium w3-margin-8">
            <h4><i class="glyphicon glyphicon-th-list"/> {{mcLabel.orderReturnsList}}</h4>
        </div>
        <div class="w3-container w3-margin-8">
            ...
            <table class="w3-table w3-striped w3-border w3-hoverable" id="mcOrderList">
                ...
                <tr v-for="item in orderItems" :key="item._id" @click.prevent="viewItem(item)">
                    ...
                    <td>
                        <p v-for="prod in item.orderItems">
                            {{productName( prod.productId )}} ({{prod.productQty}}) | {{productCost( prod.productId )}} | Cancelled?: {{prod.isCancel ? 'Yes' : 'No'}}</p>
                    </td>
...
<script>
    ...
    export default {
        ...
        computed  : {
            cancelledItems( item ) {
                return item.orderItems.filter( ( item ) => {
        ...
        methods   : {
            fetchData() {
                ...
                if( userToken ) {
                    ...
                    // orders
                    // set queryParams for cancelled orders & cancel order-items [Aug-28-2017]
                    queryParams = {
                        isCancel             : true,
                        "orderItems.isCancel": true,
                    };
            ...
            valueAmount( item ) {
                return `${item.orderPayment.tradeCurrency.toUpperCase()}${this.$num( item.orderPayment.tradeCostTotalPlusTax ).format( '0,0.00' )}`;
            },
            customerName( itemId ) {
            ...
            viewItem( item ) {
                // Set current item
                // Set current item
                this.$store.dispatch('central/setReturnItem', '');
