    ...
    <div>
        <div class="w3-container w3-blue w3-round-medium w3-margin-8">
            <h4 class="w3-left"><i class="glyphicon glyphicon-th-list"/>
                {{mcLabel.invoiceDetail}} :: {{mcLabel.orderNumber}}: {{orderView.orderNumber}}</h4>
        </div>
        <div class="w3-container w3-margin-8">
            ...
            <div>
                ...
                <hr>
                <!--TODO: Include link to productView for items-list-->
                <div>
                    <table class="w3-table w3-striped w3-border w3-hoverable" id="mcOrderViewItems">
                        ...
                        <tr v-for="item in orderItems" :key="item._id" @click.prevent="viewItem(item)">
                            ...
                            <td>
                                ...
                                <br>
                                <a id="viewItem" href="#"
                                   @click.prevent="viewItem(item.productId)">{{mcLabel.detail}} <i
    ...
    export default {
        ...
        methods   : {
            ...
            payMethod( orderNumber ){
                ...
                return payment ? `${payment.chargeInfo.source.funding.toUpperCase()} ${payment.chargeInfo.source.object.toUpperCase()} - ${payment.chargeInfo.source.brand} [****-****-****-${payment.chargeInfo.source.last4}]` : 'n/a';
            },
            viewItem( itemId ) {
        ...
        created() {
            ...
            if( Session.get( 'currentInvoiceItem' ) ) {
                this.orderView        = Session.get( 'currentInvoiceItem' );
                this.orderPayment     = Session.get( 'currentInvoiceItem' ).orderPayment;
                this.orderPayAddress  = Session.get( 'currentInvoiceItem' ).orderPayment.tradePayAddress;
                this.orderShipAddress = Session.get( 'currentInvoiceItem' ).orderPayment.tradeShipAddress;
                this.orderItems       = Session.get( 'currentInvoiceItem' ).orderItems;
            }
            // Fetch items from the server, based on role assignment / authorization
            // Get current logged-in user's token, for resource access
            this.fetchData();
