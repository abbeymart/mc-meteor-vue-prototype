<template>
    <div>
        <div class="w3-container w3-blue w3-round-medium w3-margin-8">
            <h4><i class="glyphicon glyphicon-th-list"/> {{mcLabel.invoicesList}}</h4>
        </div>
...
<script>
    ...
    import { docRoutes } from "../../../../routers/central/docRoutes";
    ...
    export default {
        ...
        methods   : {
                ...
                if( userToken ) {
                    ...
                    Meteor.call( 'getOrder', queryParams, currentItemId, userToken, ( error, result ) => {
                        if( error ) {
                            ...
                            this.isMessage   = false;
                            this.pageMessage = 'Items available';
                            this.orderItems  = this.$lo.sortBy( result.value, 'orderNumber' ).filter(item => { return item.orderPayment.tradePay === 'Invoice' });
                        } else {
                            this.isMessage   = true;
                            this.pageMessage = `${result.code}: ${result.value}`;
                        }
            ...
            viewItem( item ) {
                ...
                // Set current item
                // Session.set( 'currentInvoiceItem', item );
                // Route to detail page
