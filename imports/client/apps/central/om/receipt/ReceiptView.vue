...
<script>
    ...
    export default {
        ...
        created() {
            ...
            if( Session.get( 'currentOrderViewItem' ) ) {
                this.orderView        = Session.get( 'currentOrderViewItem' );
                this.orderPayment     = Session.get( 'currentOrderViewItem' ).orderPayment;
                this.orderPayAddress  = Session.get( 'currentOrderViewItem' ).orderPayment.tradePayAddress;
                this.orderShipAddress = Session.get( 'currentOrderViewItem' ).orderPayment.tradeShipAddress;
                this.orderItems       = Session.get( 'currentOrderViewItem' ).orderItems;
            }
            // Fetch items from the server, based on role assignment / authorization
            // Get current logged-in user's token, for resource access
            this.fetchData();
