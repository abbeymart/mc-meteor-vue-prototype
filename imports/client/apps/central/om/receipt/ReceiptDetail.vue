    ...
    <div>
        <div class="w3-container w3-blue w3-round-medium w3-margin-8">
            <h4 class="w3-left"><i class="glyphicon glyphicon-th-list"/>
                {{mcLabel.receiptDetail}} :: {{mcLabel.orderNumber}}: {{orderView.orderNumber}}</h4>
        </div>
...
<script>
    ...
    export default {
        ...
        created() {
            ...
            if( Session.get( 'currentReceiptItem' ) ) {
                this.orderView        = Session.get( 'currentReceiptItem' );
                this.orderPayment     = Session.get( 'currentReceiptItem' ).orderPayment;
                this.orderPayAddress  = Session.get( 'currentReceiptItem' ).orderPayment.tradePayAddress;
                this.orderShipAddress = Session.get( 'currentReceiptItem' ).orderPayment.tradeShipAddress;
                this.orderItems       = Session.get( 'currentReceiptItem' ).orderItems;
            }
            // Fetch items from the server, based on role assignment / authorization
            // Get current logged-in user's token, for resource access
            this.fetchData();
