    ...
    <div class="w3-container">
        ...
        <ul class="nav nav-pills w3-lightgrey">
            <li class="mcBlockLink" style="display: block;">
                <h4>{{mcLabel.actLevel}} &raquo;&raquo;</h4>
            </li>
            <router-link :to="{name: 'actLevelList'}" tag="li" class="mcBoldLink" active-class="active" exact>
                <a>{{mcLabel.list}}</a>
            </router-link>
            <!--<router-link :to="{name: 'actLevelDetail'}" tag="li" class="mcBoldLink" active-class="active">-->
                <!--<a v-if="isItem">{{mcLabel.update}}</a>-->
            <!--</router-link>-->
            <li class="mcBoldLink"><a href="#" @click.prevent="newItem">New Activity Levels</a></li>
        </ul>
        <div>
        ...
        methods   : {
            newItem(){
                ...
                if( parts[ lastIndex ] === 'new' ) {
                    ...
                    this.$router.push( { name: 'actLevelNew' } );
                }
            },
        },
        created(){
