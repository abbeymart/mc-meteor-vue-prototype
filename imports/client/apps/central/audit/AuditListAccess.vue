<template>
    <div>
        ...
        <div class="w3-container w3-margin-8">
            ...
            <table class="w3-table w3-striped w3-border w3-hoverable" id="mcAuditListAccess">
                <thead>
                <tr class="w3-red">
                    <th scope="col">{{mcLabel.collValue}}</th>
                    <th scope="col">{{mcLabel.actionType}}</th>
...
<script>
    ...
    export default {
            ...
            if( userToken ) {
                ...
                Meteor.call( 'getAudit', queryParams, currentItemId, userToken, ( error, result ) => {
                    if( error ) {
                        ...
                        this.isMessage   = false;
                        this.pageMessage = 'Items available (audit)';
                        this.auditItems  = this.$lo.sortBy( result.value, 'collName' ).filter( ( item ) => {
                            return (item.actionType === 'Login' || item.actionType === 'Logout');
                        } );
                    } else {
                        this.isMessage   = true;
                        this.pageMessage = `${result.code}: ${result.value}`;
                ...
                Meteor.call( 'getLang', queryParams, currentItemId, userToken, ( error, result ) => {
                    if( error ) {
                        ...
                        this.isMessage   = false;
                        this.pageMessage = 'Items available (lang)';
                ...
                Meteor.call( 'getCode', queryParams, currentItemId, userToken, ( error, result ) => {
                    if( error ) {
                        ...
                        this.isMessage   = false;
                        this.pageMessage = 'Items available (code)';
        ...
        updated() {
            document.getElementById( 'mcAuditListAccess' ).DataTable({
                destroy     : true,
