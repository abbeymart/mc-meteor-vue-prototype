<template>
    <div>
        ...
        <div class="w3-container w3-margin-8">
            ...
            <table class="w3-table w3-striped w3-border w3-hoverable" id="mcAuditListUpdate">
                <thead>
                <tr class="w3-red">
                    <th scope="col">{{mcLabel.coll}}</th>
                    <th scope="col">{{mcLabel.oldValue}}</th>
                    <th scope="col">{{mcLabel.newValue}}</th>
                ...
                <tr v-for="item in auditItems" :key="item._id" @click.prevent="updateItem(item)">
                    <td>{{item.collName}}</td>
                    <td>{{valueDetail(item.collOldValues)}}</td>
                    <td>{{valueDetail(item.collNewValues)}}</td>
...
<script>
    ...
    export default {
        ...
        updated() {
            document.getElementById( 'mcAuditListUpdate' ).DataTable({
                destroy     : true,
