...
WebApp.connectHandlers.use( '/api/services', ( req, res, next ) => {
    ...
    // security checks: ensure valid token || TODO: permission-code for non-authenticated access
    ...
    switch( req.method ) {
        case 'GET':
            result = Meteor.call( 'getService', itemParams, currentItemId, token, skip, limit );
        ...
        case 'POST':
        case 'PUT':
            result = Meteor.call( 'saveService', itemParams, currentItemId, token );
            // check result and perform next action or send error message
            if( !_.isEmpty( result ) ) {
                apiUtils.response( res, result.resCode, result );
            } else {
            ...
            break;
        case 'DELETE':
            result = Meteor.call( 'removeService', currentItemId, token, );
            if( !_.isEmpty( result ) ) {
                apiUtils.response( res, result.resCode, result );
            } else {
                apiUtils.response( res, 404, `${mcMessage.noInfo}` );
